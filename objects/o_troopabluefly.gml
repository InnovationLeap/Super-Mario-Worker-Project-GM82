...
if global.pauza=0 && global.etappokonany=0 {
    ...
    if aktywowany=1 {
        ...
        // chodzenie
        if kierunek=1 {
            if (place_meeting(x+1,y,obj_wall)|| place_meeting(x+1,y,o_pointblock)) {kierunek=-1;x-=1;image_xscale=-1} else {
                x+=1;
            }
        } else {
            if (place_meeting(x-1,y,obj_wall)|| place_meeting(x-1,y,o_pointblock)) {kierunek=1;x+=1;image_xscale=1} else {
    ...
    if rodzajzabicia=7 {instance_destroy(); redduj=instance_create(x,y-1,o_troopashell2);redduj.sprite_index=s_troopablueshell;redduj.is_flipped=1;redduj.grawitacja=-11;redduj.czerwona=2;redduj.tail_kicked=1;redduj.kierunek=kierunek; instance_create(x,y,o_punkciornik)}
