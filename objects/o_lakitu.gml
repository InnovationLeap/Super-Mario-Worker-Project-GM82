...
if global.pauza=0 && global.level_complete=0 {
    ...
    if activated=1 {
        ...
        if attack_timer>=300 && attack_timer<1000 {attack_timer=1000; reload_timer=random(1)+1; sprite_index=s_lakitu2; image_index=0; animacja=2000}
        ...
        if attack_timer=2010 {attack_timer=0; sprite_index=s_lakitu; image_index=0; instance_create(x,y-20,o_lakitubomb); animacja=0;avy=1 or 2 or 3}
        //音效
    ...
    if kill_type=1 {instance_destroy(); instance_create(x,y,o_lakitudead); instance_create(x,y,o_scorepop)}
    if kill_type=3 || kill_type=4 || kill_type=5 {instance_destroy(); instance_create(x,y,o_lakitudead); tmp=instance_create(x,y,o_scorepop); tmp.image_index=0;if global.sample=1 {tmp2=sound_play(snd_kick);sound_volume(snd_kick,global.game_volume)}}
    if kill_type=2 {instance_destroy(); instance_create(x,y,o_lakitudead);}
