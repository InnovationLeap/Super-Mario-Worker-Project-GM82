...
if global.pauza=0 && global.level_complete=0 {
    ...
    if !place_meeting(x,y,obj_wall) && !place_meeting(x,y,o_pointblock) {
        ...
        if x<view_xview[0]+740 && x>view_xview[0]-100 && y>view_yview[0]-64 && y<view_yview[0]+480+64 shoot_seq+=1+shoot_seq2
        if shoot_seq<=prepare_time {sprite_index=s_firesis}
        if shoot_seq>prepare_time {sprite_index=s_firesisfire}
        if shoot_seq>prepare_time+shoot_time {shoot_seq=0; tmp=instance_create(x,y+8,o_fff); tmp.dir=image_xscale;sound_play(snd_fire);prepare_time=irandom_range(50,85);shoot_time=irandom_range(50,60);}
    ...
    if kill_type=1 {instance_destroy(); instance_create(x,y,o_FSdead); instance_create(x,y,o_scorepop)}
    if kill_type=5 || kill_type=4 || kill_type=3 {instance_destroy(); instance_create(x,y,o_FSdead); tmp=instance_create(x,y,o_scorepop); tmp.image_index=0;if global.sample=1 {tmp2=sound_play(snd_kick);sound_volume(snd_kick,global.game_volume)}}
    if kill_type=2 {instance_destroy(); instance_create(x,y,o_FSdead);}
