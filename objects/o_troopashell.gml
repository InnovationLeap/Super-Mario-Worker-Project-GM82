#define Create_0
/*"/*'/**//* YYD ACTION
lib_id=1
action_id=603
applies_to=self
*/
hardshell=0
grav=0
state=0

image_speed=0
dir=-1
activated=0
kill_type=0
nabijanie=0
killer=0 // czy mozna rozdeptywac 0 - tak, 1 - nie, 2 - tak ale nie zabija,
knockback=0 // przy rozdeptywaniu ile ma zwiekszyc odskok

shell_kind=0


hurt_delay=10
fangsi=0
single=0
is_flipped=0

// 发光位置微调
light_x = 0;
light_y = 0;
light_radius = 1;
#define Step_0
/*"/*'/**//* YYD ACTION
lib_id=1
action_id=603
applies_to=self
*/
/*备忘
o_pointblock.podbity=1时被激活
*/
//运动龟壳
if activated=0 {
    if x>view_xview[0]-100 && x<view_xview[0]+740 && y>view_yview[0]-100 && y<view_yview[0]+580 {
        activated=1
        if single && x<o_marker.x {dir=1}
        if single && x>=o_marker.x {dir=-1}
    }
}

if global.pauza=0 && global.level_complete=0 {
    // is_flipped drives image_yscale (upside-down state)
    if is_flipped=1 {image_yscale=-1} else {image_yscale=1}
    if hurt_delay>0 {hurt_delay-=1}
    if activated=1 {
        // spadanie
        if shell_kind=1 {sprite_index=s_trooparedshell}
        if shell_kind=4 {
            if sprite_index != s_spinyshell {sprite_index = s_spinyshell; image_index = 1}
            hardshell=0; shell_proof=1}
/*
basic_movement(5,0.4,0,0);

if hardshell=1 {
if place_meeting(x+5,y,o_pointblock) && kierunek>0 {lolo=instance_place(x+5,y,o_pointblock);lolo.podbity=1;if lolo.bonus=20{lolo.shaben=1;lolo.aplikacja=10};fangsi=1;kierunek=kierunek*-1}
if place_meeting(x-5,y,o_pointblock) && kierunek<0 {lolo=instance_place(x-5,y,o_pointblock);lolo.podbity=1;if lolo.bonus=20{lolo.shaben=1;lolo.aplikacja=10};fangsi=1;kierunek=kierunek*-1}
if place_meeting(x+5,y,o_pointblockpodbijacz) && kierunek>0 {lolo=instance_place(x+5,y,o_pointblockpodbijacz);lolo.podbity=1;fangsi=1;kierunek=kierunek*-1}
if place_meeting(x-5,y,o_pointblockpodbijacz)&& kierunek<0 {lolo=instance_place(x-5,y,o_pointblockpodbijacz);if(!lolo.podbity){lolo.podbity=1;}fangsi=1;kierunek=kierunek*-1}
//if (place_meeting(x+5,y,obj_wall) && !place_meeting(x+5,y,o_pointblockpodbijacz)) || (place_meeting(x-5,y,obj_wall) && !place_meeting(x-5,y,o_pointblockpodbijacz))  {kierunek=kierunek*-1}
}
*/

        if state=0 && !place_meeting(x,y+1,obj_halfground) && !place_meeting(x,y+1,obj_wall) && !place_meeting(x,y+1,o_pointblock) {state=1}
        if state=1 {grav+=0.5; y+=grav}
        // ceiling: if moving up and inside a block, push down out of it
        if grav<0 {
            while place_meeting(x,y,obj_halfground) || place_meeting(x,y,obj_wall) || place_meeting(x,y,o_pointblock) || place_meeting(x,y,obj_waall) || place_meeting(x,y,o_breakblock) || place_meeting(x,y,o_pointblock2) || place_meeting(x,y,obj_static) {y+=1; grav=0}
        }
        if state=1 && (place_meeting(x,y+1,obj_halfground) || place_meeting(x,y+1,obj_wall) || place_meeting(x,y+1,o_pointblock)) {grav=0; state=2;}
        while state=2 && (place_meeting(x,y,obj_halfground) ||place_meeting(x,y,obj_wall) || place_meeting(x,y,o_pointblock)) { y-=1 if (!place_meeting(x,y,obj_halfground) && !place_meeting(x,y,obj_wall) && !place_meeting(x,y,o_pointblock)) {state=0}}

        if hardshell=1 {
            if place_meeting(x+5,y,o_pointblock) && dir>0 {tmp=instance_place(x+5,y,o_pointblock);tmp.bumped=1;if tmp.bonus=20 {tmp.shaben=1;tmp.spawn_delay=10};fangsi=1;dir=dir*-1}
            if place_meeting(x-5,y,o_pointblock) && dir<0 {tmp=instance_place(x-5,y,o_pointblock);tmp.bumped=1;if tmp.bonus=20 {tmp.shaben=1;tmp.spawn_delay=10};fangsi=1;dir=dir*-1}
            if place_meeting(x+5,y,o_blockbumper) && dir>0 {tmp=instance_place(x+5,y,o_blockbumper);tmp.bumped=1;fangsi=1;dir=dir*-1}
            if place_meeting(x-5,y,o_blockbumper)&& dir<0 {tmp=instance_place(x-5,y,o_blockbumper);if(!tmp.bumped) {tmp.bumped=1;}fangsi=1;dir=dir*-1}
        }

        if dir=-1 && (!place_meeting(x-5,y,obj_wall) || !place_meeting(x-5,y,o_pointblock)) {x-=5}
        if dir=1 && (!place_meeting(x+5,y,obj_wall) || !place_meeting(x+5,y,o_pointblock)) {x+=5}

        if hardshell=1 {
            if (place_meeting(x+5,y,obj_wall) && !place_meeting(x+5,y,o_blockbumper)) || (place_meeting(x-5,y,obj_wall) && !place_meeting(x-5,y,o_blockbumper)) {dir=dir*-1}
        } else {
            if  place_meeting(x+5,y,obj_wall) || place_meeting(x-5,y,obj_wall) || place_meeting(x+5,y,o_pointblock) || place_meeting(x-5,y,o_pointblock) {dir=dir*-1}
        }

        image_index+=0.4

    }

    // uppercut i zwykla smierc
    // tail attack: flip + kick moving shell (no destroy, just modify state)
    if kill_type=7 {
        is_flipped = 1
        grav = -11
        hurt_delay = 10
        state = 1
        if x < o_marker.x { dir = -1 } else { dir = 1 }
        kill_type = 0
    }

    if hardshell=0 {

        //if place_meeting(x,y,o_uppercut) {energia-=3; rodzajzabicia=0}
        if kill_type=6 {instance_destroy(); tmp6=instance_create(x,y,o_troopashell2); tmp6.dir=dir; tmp6.shell_kind=shell_kind; tmp6.is_flipped=is_flipped; if shell_kind=1 {tmp6.sprite_index=s_trooparedshell}  ; if shell_kind=2{tmp6.sprite_index=s_troopablueshell} ;if shell_kind=3{tmp6.sprite_index=s_troopashellgold} ;if shell_kind=4{tmp6.sprite_index=s_spinyshell}}
        if kill_type=2 {instance_destroy(); tmp=instance_create(x,y,o_troopadead); tmp.shell_kind=shell_kind; if shell_kind=1 {tmp.sprite_index=s_trooparedshell} ; if shell_kind=2{tmp.sprite_index=s_troopablueshell} ;if shell_kind=3{tmp.sprite_index=s_troopashellgold} ;if shell_kind=4{tmp.sprite_index=s_spinyshelldead}}
        if kill_type=1 {instance_destroy(); tmp6=instance_create(x,y,o_troopashell2); tmp=instance_create(x,y,o_scorepop); tmp6.dir=dir; tmp6.shell_kind=shell_kind; tmp6.is_flipped=is_flipped; if shell_kind=1 {tmp6.sprite_index=s_trooparedshell}  ; if shell_kind=2{tmp6.sprite_index=s_troopablueshell} ;if shell_kind=3{tmp6.sprite_index=s_troopashellgold} ;if shell_kind=4{tmp6.sprite_index=s_spinyshell}}
        if kill_type=3 || kill_type=4 || kill_type=5 {instance_destroy(); tmp6=instance_create(x,y,o_troopadead); tmp=instance_create(x,y,o_scorepop); tmp6.shell_kind=shell_kind; if shell_kind=1 {tmp6.sprite_index=s_trooparedshell}  ; if shell_kind=2{tmp6.sprite_index=s_troopablueshell};if shell_kind=3{tmp6.sprite_index=s_troopashellgold};if shell_kind=4{tmp6.sprite_index=s_spinyshelldead};if global.sample=1{tmp2=sound_play(snd_kick);sound_volume(snd_kick,global.game_volume)}}

    } else {
        //if place_meeting(x,y,o_uppercut) && fangsi=0 {energia-=23333333333333333; rodzajzabicia=0}//顶

        if kill_type=1 {
            instance_destroy();
            tmp=instance_create(x,y,o_troopashell2);
            tmp.sprite_index=s_buzzyshell;
            tmp.dir=dir;
            tmp.shell_proof=1
            tmp.hardshell=1
            tmp.is_flipped=is_flipped;
            tmp.shell_kind=shell_kind;
            instance_create(x,y,o_scorepop)}

        if kill_type=6 {
            instance_destroy();
            tmp=instance_create(x,y,o_troopashell2);
            tmp.sprite_index=s_buzzyshell;
            tmp.dir=dir;
            tmp.shell_proof=1
            tmp.hardshell=1
            tmp.is_flipped=is_flipped;
            tmp.shell_kind=shell_kind;
        }

        if kill_type=5 {
            instance_destroy();
            tmp2=instance_create(x,y,o_troopadead);
            if shell_kind=4 {tmp2.sprite_index=s_spinyshelldead} else {tmp2.sprite_index=s_buzzyshell}
            tmp=instance_create(x,y,o_scorepop);
            tmp.image_index=0;
            if global.sample=1 {tmp2=sound_play(snd_kick);sound_volume(snd_kick,global.game_volume)}}//顶

        if kill_type=2 {
            instance_destroy();
            tmp=instance_create(x,y,o_troopadead);
            if shell_kind=4 {tmp.sprite_index=s_spinyshelldead} else {tmp.sprite_index=s_buzzyshell}}

    }

    // zabijanie od muszli
    //先把这里治好

    if sprite_index != s_buzzyshell {

        if place_meeting(x,y,o_goomba) && !place_meeting(x,y,o_troopashell) && !place_meeting(x,y,o_fishblue) && !place_meeting(x,y,o_fishyellow) {
            tmp=instance_place(x,y,o_goomba)
            if tmp.stomp_proof=0 && tmp.shell_proof=0 && tmp.noshellkick=0 {
                tmp.kill_type=2
                nabijanie+=1
                tmp2=instance_create(x,y,o_scorepop2)
                tmp2.image_index=nabijanie-1
                if nabijanie>6 {nabijanie=0}
            }
            if tmp.shell_proof <> 0 && tmp.noshellkick=0 {
                // normal shell hitting static spiny shell: kill the static spiny, not self
                if tmp.object_index == o_troopashell2 && tmp.shell_kind == 4 {
                    tmp.kill_type = 2
                    tmp2=sound_play(snd_kick);sound_volume(snd_kick,global.game_volume)
                } else { kill_type=2 ;tmp2=sound_play(snd_kick);sound_volume(snd_kick,global.game_volume)}
            }

        }

        if place_meeting(x,y,o_troopashell) {
            tmp=instance_place(x,y,o_troopashell)
  /*  if lolo.dabusi=0 && lolo.dasini=0 {
  //  lolo.energia-=10000000
    lolo.rodzajzabicia=2
    nabijanie+=1
    fofo=instance_create(x,y,o_punkciornik2)
    fofo.image_index=nabijanie-1
    if nabijanie>6 {nabijanie=0}
    energia-=1
    rodzajzabicia=2
}*/
            if tmp.shell_proof <> 0 { kill_type=2 ;tmp2=sound_play(snd_kick);sound_volume(snd_kick,global.game_volume)}

        }

    } else {

        if place_meeting(x,y,o_goomba) && !place_meeting(x,y,o_troopashell) /*&& !place_meeting(x,y,o_rybekc) && !place_meeting(x,y,o_rynekd)*/
        {
            tmp=instance_place(x,y,o_goomba)
            if tmp.stomp_proof=0 && tmp.noshellkick=0 {
                // lolo.energia-=233333333333333333333
                tmp.kill_type=2
                nabijanie+=1
                tmp2=instance_create(x,y,o_scorepop2)
                tmp2.image_index=nabijanie-1
                if nabijanie>6 {nabijanie=0}
            }
        }

        if place_meeting(x,y,o_troopashell) {
            tmp=instance_place(x,y,o_troopashell)
            if tmp.stomp_proof=0 && tmp.shell_proof=0 {
                // lolo.energia-=233333333333333333333
                tmp.kill_type=2
                nabijanie+=1
                tmp2=instance_create(x,y,o_scorepop2)
                tmp2.image_index=nabijanie-1
                if nabijanie>6 {nabijanie=0}
            }
/*if lolo.zarabia=1 && lolo.energia>0 { //WTF，这干嘛的，这代码有存在意义？？？？？
    lolo.energia-=233333333333333333333
    lolo.rodzajzabicia=2
    nabijanie+=1
    fofo=instance_create(x,y,o_punkciornik2)
    fofo.image_index=nabijanie-1
    if nabijanie>6 {nabijanie=0}
    energia-=233333333333333333333
    rodzajzabicia=2
    }*///直接禁了（

        }

    }

    // niszcz po za ekranem
    if y>room_height[0]+64 {instance_destroy()}

    if fangsi>0 {fangsi+=1}
    if fangsi>7 {fangsi=0}
}


// 硬壳（划掉）所有龟壳的碰撞检测
if place_meeting(x, y, o_piranhablue) || place_meeting(x, y, o_piranhablueflip) {
    if place_meeting(x, y, o_piranhablue) {gay_fa = instance_place(x,y,o_piranhablue)}
    if place_meeting(x, y, o_piranhablueflip) {gay_fa = instance_place(x,y,o_piranhablueflip)}

    if gay_fa.dasini_lake = 1 {

        // 设置死亡类型为特殊击杀（对应硬壳的5号死亡方式）
        kill_type = 1;

        // 死亡处理（保持与原有逻辑一致）
        instance_destroy();
        tmp2 = instance_create(x, y, o_troopadead);
        tmp2.shell_kind = shell_kind;
        tmp2.hardshell = hardshell;
        if shell_kind=4 {tmp2.sprite_index = s_spinyshelldead;} else if tmp2.hardshell = 1 {tmp2.sprite_index = s_buzzyshell;} else {
            if shell_kind=1 {tmp2.sprite_index=s_trooparedshell} ; if shell_kind=2{tmp2.sprite_index=s_troopablueshell} ;if shell_kind=3{tmp2.sprite_index=s_troopashellgold}
        }

        // 创建分数
        tmp = instance_create(x, y, o_scorepop);
        tmp.image_index = 0;

        /*
        // 音效反馈（根据全局设置）
        if global.sample == 1 {
            sound_play(snd_kick);
            sound_volume(snd_kick, global.glosnosc);
        }
        */

    }
}
