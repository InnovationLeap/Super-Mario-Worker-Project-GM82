...
if global.pauza=0 {
    ...
    if hele = 1 {
        ...
        if sekwencja=1 {
            if  kierunek=1 {
                if (place_meeting(x+2,y,obj_wall) or place_meeting(x+2,y,o_pointblock)) {kierunek=-1; if grawsek=0 {odbicie=1}}
            } else {
                if (place_meeting(x-2,y,obj_wall) or place_meeting(x-2,y,o_pointblock)) {kierunek=1; if grawsek=0 {odbicie=1}}
            }
            if odbicie=0 {x+=2*kierunek}
        }
        ...
        if grawsek=1 {
            ...
            if (place_meeting(x,y+1,obj_halfground) || place_meeting(x,y,obj_wall) || place_meeting(x,y,o_pointblock)) {grawsek=2; grawitacja=0; spadaczanim=1}
        }
...
applies_to=self
*/
if kierunek=-1 {draw_sprite_ext(s_bonus1up,animacja2,x+sekwodbicia*1.6,y+spadaczanim2*1.6,1-sekwodbicia/20,1-spadaczanim2/20,0,c_white,1)}
if kierunek=1 {draw_sprite_ext(s_bonus1up,animacja2,x,y+spadaczanim2*1.6,1-sekwodbicia/20,1-spadaczanim2/20,0,c_white,1)}
