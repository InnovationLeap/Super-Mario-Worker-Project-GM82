...
if global.pauza=0 && global.level_complete=0 {
    ...
    activated=0
    if x>view_xview[0]-100 && x<view_xview[0]+740 && y>view_yview[0]-100 && y<view_yview[0]+580 {activated=1}
    if x>o_marker.x-80 && x<o_marker.x+80 && crush_state=0 {crush_state=1}
    if crush_state=1 && !place_meeting(x,y,obj_halfground) && !place_meeting(x,y,obj_wall) && !place_meeting(x,y,o_pointblock) {vy_offset+=1; y+=vy_offset}
    if crush_state=1 && (place_meeting(x,y,obj_halfground) ||  place_meeting(x,y,obj_wall) || place_meeting(x,y,o_pointblock)) {crush_state=2; vy_offset=0;if y>view_yview[0]-64 && y<view_yview[0]+480+64 {sound_play(snd_stun2)} }
    ...
    while crush_state=2 && (place_meeting(x,y,obj_halfground) ||  place_meeting(x,y,obj_wall) || place_meeting(x,y,o_pointblock)) {
        // 可选项：石盾触发砖块
...
while (brick != noone) {
    ...
    if crush_state=2 {crush_state=3}
    if crush_state>=3 && crush_state<100 {crush_state+=1}
    if crush_state>=100 && y>starty {
        y -= 1;
        stun = false;
    }
    if crush_state>=100 && y<=starty {crush_state=0}
...
if place_meeting(x,y,o_marker) && o_marker.shell_lock=1 && zabil=0 {zabil=1; xiao=1}
if place_meeting(x,y,o_marker)&& global.pauza=2 {xiao=0.5}
if xiao=0.5 && global.pauza=0 {xiao=1}
if xiao=1 {sound_play(snd_thwomp);sprite_index=sprite363;image_speed=0.1;xiao=2}
if xiao=2 && !sound_isplaying(snd_thwomp) && image_index=11 {image_speed=0;image_index=0;sprite_index=sprite157;xiao=0}
