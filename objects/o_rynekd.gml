...
if is_swimming=1 && y<=global.poziomwody{
  fofo=instance_create(x,y,o_rybekd3);
  fofo.xspeed=2-irandom(2)+1.25
  fofo.yspeed=-3-1.3*irandom(4)
  instance_destroy();
}
/*"/*'/**//* YYD ACTION
lib_id=1
...
if aktywowany=0 {if x>view_xview[0]-100 && x<view_xview[0]+740 && y>view_yview[0]-100 && y<view_yview[0]+580
    {
    if x<o_marker.x {aktywowany=1;}
    if x>=o_marker.x {aktywowany=1;}
    }}
if aktywowany=1{
// chodzenie

if x>o_marker.x {x-=randomus; kierunek=-1}
if x<o_marker.x {x+=randomus; kierunek=1}
if y>o_marker.y && y>global.poziomwody+32 {y-=randomus;}
