...
if global.pauza=0 {
    ...
    if hele = 1 {
        ...
        if sekwencja=0 {
            if(place_meeting(x,y,obj_wall) || place_meeting(x,y,o_pointblock)) {y-=1} else {sekwencja=1}
        }
        ...
        if grawsek=0 && !place_meeting(x,y+1,obj_halfground) && !place_meeting(x,y+1,obj_wall) && !place_meeting(x,y+1,o_pointblock) {grawsek=1}
        if grawsek=1 && grawitacja<7 {grawitacja+=0.5}
        if grawsek=1 && !place_meeting(x,y-8,obj_wall) && !place_meeting(x,y-8,o_pointblock) && grawitacja<0 y+=grawitacja
        if grawsek=1 && grawitacja>0 y+=grawitacja
        if grawsek=1 && (place_meeting(x,y+1,obj_halfground) || place_meeting(x,y,obj_wall) || place_meeting(x,y,o_pointblock)) {grawsek=2; grawitacja=0; spadaczanim=1}
        while grawsek=2 && (place_meeting(x,y,obj_halfground) || place_meeting(x,y,obj_wall) || place_meeting(x,y,o_pointblock)) {y-=1}
        if grawsek=2 && !place_meeting(x,y,obj_halfground) && !place_meeting(x,y,obj_wall) && !place_meeting(x,y,o_pointblock) {grawsek=0}
