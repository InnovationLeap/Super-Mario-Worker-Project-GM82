...
if sekwencja<100
    {
    sekwencja+=1
    if strzal>0 && przeladowanie>15
        {
        przeladowanie=0; strzal-=1; fofoz=instance_create(x,y+30,o_enemyfire);fofoz.iyor=2
        if global.sample=1 {fofo=sound_play(snd_fire);sound_volume(snd_fire,global.glosnosc);dzwiek=1}
        }
    if x>view_xview[0]-100 && x<view_xview[0]+740 && y>view_yview[0]-100 && y<view_yview[0]+580 {przeladowanie+=1}
    }
...
if sekwencja=351 {sekwencja=0;strzal=3}
}
...
if rodzajzabicia=3 || rodzajzabicia=4 || rodzajzabicia=5 {instance_destroy();lolo=instance_create(x,y,o_punkciornik); lolo.image_index=0;if global.sample=1 {fofo=sound_play(snd_kick);sound_volume(snd_kick,global.glosnosc)}}
