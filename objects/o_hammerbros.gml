...
// strzelanie
    ...
    if x<o_marker.x {image_xscale=1}
    if x<view_xview[0]+740 && x>view_xview[0]-100 sekwencjashot+=1+sekwencjashot2
    if sekwencjashot<=150 {sprite_index=s_malpka}
    if sekwencjashot>150 {sprite_index=s_malpkazmlotkiem}
    if sekwencjashot>250 {sekwencjashot=0; lolo=instance_create(x,y,o_mlotekmalpki); lolo.kierunek=image_xscale;sound_play(snd_mlotek)}
...
if rodzajzabicia=1 {instance_destroy(); instance_create(x,y,o_hammerbrosdead); instance_create(x,y,o_punkciornik)}
if rodzajzabicia=3 || rodzajzabicia=4 || rodzajzabicia=5 {instance_destroy(); instance_create(x,y,o_hammerbrosdead); lolo=instance_create(x,y,o_punkciornik); lolo.image_index=0;if global.sample=1 {fofo=sound_play(snd_kick);sound_volume(snd_kick,global.glosnosc)}}
if rodzajzabicia=2 {instance_destroy(); instance_create(x,y,o_hammerbrosdead);}
