...
if global.pauza=0 && global.level_complete=0 {
    ...
    if activated=1 {
        ...
        if state<100 {
            state+=1
            if shot_timer>0 && reload>15 {
                reload=0; shot_timer-=1; tmp7=instance_create(x,y+30,o_enemyfire);tmp7.vy_offset=2
                if global.sample=1 {tmp2=sound_play(snd_fire);sound_volume(snd_fire,global.game_volume);sound_played=1}
            }
            if x>view_xview[0]-100 && x<view_xview[0]+740 && y>view_yview[0]-100 && y<view_yview[0]+580 {reload+=1}
        }
        ...
        if state=351 {state=0;shot_timer=3}
    }
    ...
    if kill_type=3 || kill_type=4 || kill_type=5 {instance_destroy();tmp=instance_create(x,y,o_scorepop); tmp.image_index=0;if global.sample=1 {tmp2=sound_play(snd_kick);sound_volume(snd_kick,global.game_volume)}}
