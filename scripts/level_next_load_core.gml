// level_next_load_core(argument0=读取模式: 1=Play Scenario 内存数组(o_readsmwpfile), 0=单关文件句柄(global.toload))
// Next_level_Load 与 Next_level_Load2 的合并主体（自 HEAD 版机械归一化生成）。
// 读取机制差异收敛到 level_read_peek/next/eof 三个原语；
// 初始游标推进、lastlev 守卫、checkpoint 重置策略、loadingsav 收尾保留在两个薄壳中。
global.ll_array = argument0
...
tmp3=0;
//show_message(string(o_readsmwpfile.toloadpointer)+":"+level_read_peek())
room_set_width(Play_Room,real(level_read_peek()))
level_read_next()
//show_message(string(o_readsmwpfile.toloadpointer)+":"+level_read_peek())
room_set_height(Play_Room,real(level_read_peek()))
global.level_name_play=level_read_next()
global.level_author=level_read_next()
level_read_next()
global.level_time=real(level_read_peek())
level_read_next()
global.level_gravity=real(level_read_peek())
level_read_next()
global.level_bowser_hp=real(level_read_peek())
global.bowser_hp_fixed=global.level_bowser_hp
level_read_next()
global.water_level=real(level_read_peek())
...
if level_read_peek()='BlocksDataStart' {
aa=level_read_next()
    ad=0
    while aa!='BlocksEnd' {
...
while !level_read_eof() && aa!='New Level' {
aa=level_read_next()
...
if ai=20 {ae=room_instance_add(Play_Room,transA(string_copy(aa,4,4)),transA(string_copy(aa,8,4)),o_checkpoint);skript=string_insert('('+string(ae)+').checknumber='+string(checka)+';',skript,string_length(skript)+1);
    //检查点数组重置策略：数组(Play Scenario)模式尊重 loadingsav；文件(单关)模式恒重置——与两份旧版逐一对齐
    if global.ll_array = 1 {
        if global.loadingsav = 0 {
            global.check[checka,0]=0
            global.check[checka,1]=0
        }
    } else {
        global.check[checka,0]=0
        global.check[checka,1]=0
...
if level_read_eof() {global.lastlev=1}
