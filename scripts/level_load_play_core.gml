// level_load_play_core(argument0=主目标房间, argument1=扩展名'.smwlx'/'.mflx',
//                       argument2=是否解析is_orange字段, argument3=遗留交叉目标房间,
//                       argument4=强滚速度是否除以100)
// Load_Script_Play 与 Load_Script_Play2 的合并主体（自 HEAD 版逐行机械变换生成）。
// 两版全部内容差异（规范化穷尽比对确认）：
//   1) 扩展名与主房间 —— argument0/1；
//   2) 运输桥 is_orange 字段解析仅 .smwl 有 —— argument2 门控；
//   3) 强滚(scrollspeed) .smwl 除以100且语句带尾分号、.mflx 原值无分号 —— argument4 控制 _sd 后缀与分号还原；
//   4) 【祖传行为·疑似bug】景物(o_scenery)、场景切换元件(o_bgmchange)、缺马里奥报错兜底(o_marker)
//      在旧版 Play2 中仍写入 Play_Room 而非 Play_Room2 —— 原样保留于 argument3，两个包装层现都传 Play_Room。
if file_exists(filename_change_ext(global.toload,argument1)) {file_delete(filename_change_ext(global.toload,argument1))}
GZ_DeCompressFile(global.toload,filename_change_ext(global.toload,argument1))
global.toload=filename_change_ext(global.toload,argument1)
...
room_set_width(argument0,real(file_text_read_string(global.toload)))
file_text_readln(global.toload)
room_set_height(argument0,real(file_text_read_string(global.toload)))
file_text_readln(global.toload)
...
zxy=room_instance_add(argument0,0,0,o_preview)
ah=0
draw_set_alpha(1)
...
while !file_text_eof(global.toload) {
    ...
    if string_char_at(aa,1)='0' {
        ae=room_instance_add(argument0,transA(string_copy(aa,4,4)),transA(string_copy(aa,8,4)),v_ens[real(string_copy(aa,2,2))])
    ...
    if string_char_at(aa,1)='2' {
        ...
        if ai=17 {ae=room_instance_add(argument0,transA(string_copy(aa,4,4))+32,transA(string_copy(aa,8,4))+32,o_exitar)}
        if ai=18 {ae=room_instance_add(argument0,transA(string_copy(aa,4,4)),transA(string_copy(aa,8,4)),obj_wall)}
        if ai=19 && ah=0 {ah=1;ae=room_instance_add(argument0,transA(string_copy(aa,4,4)),transA(string_copy(aa,8,4)),o_marker)}
        if ai=20 {ae=room_instance_add(argument0,transA(string_copy(aa,4,4)),transA(string_copy(aa,8,4)),o_checkpoint);skript=string_insert('('+string(ae)+').checknumber='+string(checka)+';',skript,string_length(skript)+1);global.check[checka,0]=0;global.check[checka,1]=0;checka+=1}
        if ai<17 {ae=room_instance_add(argument0,transA(string_copy(aa,4,4)),transA(string_copy(aa,8,4)),o_windas);skript=string_insert('('+string(ae)+').type='+string(ai)+';',skript,string_length(skript)+1)}
        if ai=21 {ae=room_instance_add(argument0,transA(string_copy(aa,4,4)),transA(string_copy(aa,8,4)),o_windas);skript=string_insert('('+string(ae)+').type='+string(17)+';',skript,string_length(skript)+1)}
        if ai=22 {ae=room_instance_add(argument0,transA(string_copy(aa,4,4)),transA(string_copy(aa,8,4)),obj_waall)}
        if ai=23 {ae=room_instance_add(argument0,transA(string_copy(aa,4,4)),transA(string_copy(aa,8,4)),o_scroll);skript=string_insert('('+string(ae)+').scrollspeed='+string(transA(string_copy(aa,12,3)))+_sd+';',skript,string_length(skript)+1)
            if argument2 = 1 && string_length(aa)>=15 {skript=string_insert('('+string(ae)+').is_orange='+string_copy(aa,15,1)+';',skript,string_length(skript)+1)}
            ds_list_add(global.autoscrolls,ae)}//恶劣强滚
        if ai=24 {ae=room_instance_add(argument0,transA(string_copy(aa,4,4)),transA(string_copy(aa,8,4)),o_onlyU)}
        if ai=25 {ae=room_instance_add(argument0,transA(string_copy(aa,4,4)),transA(string_copy(aa,8,4)),o_windas);skript=string_insert('('+string(ae)+').type='+string_copy(aa,12,3)+'+6*'+string_copy(aa,15,3)+';',skript,string_length(skript)+1)}
        if ai=26 {ae=room_instance_add(argument0,transA(string_copy(aa,4,4)),transA(string_copy(aa,8,4)),o_messageblock);_msg=string_copy(aa,12,9999);_msg=string_replace_all(_msg,"\#",chr(2));_msg=string_replace_all(_msg,"#",chr(13));_msg=string_replace_all(_msg,chr(2),"#");ds_map_add(global.msgMap,ae,_msg);skript=string_insert('('+string(ae)+').textMessage=ds_map_find_value(global.msgMap,'+string(ae)+')',skript,string_length(skript)+1)}
        if ai=27 {ae=room_instance_add(argument0,transA(string_copy(aa,4,4)),transA(string_copy(aa,8,4)),o_switch);skript=string_insert('('+string(ae)+').color='+string_copy(aa,12,1)+';',skript,string_length(skript)+1)}
        if ai=28 {ae=room_instance_add(argument0,transA(string_copy(aa,4,4)),transA(string_copy(aa,8,4)),o_yinyang);skript=string_insert('('+string(ae)+').color='+string_copy(aa,12,1)+';'+'('+string(ae)+').yin=1;',skript,string_length(skript)+1)}
        if ai=29 {ae=room_instance_add(argument0,transA(string_copy(aa,4,4)),transA(string_copy(aa,8,4)),o_yinyang);skript=string_insert('('+string(ae)+').color='+string_copy(aa,12,1)+';'+'('+string(ae)+').yin=0;',skript,string_length(skript)+1)}
        if ai=30 {ae=room_instance_add(argument0,transA(string_copy(aa,4,4)),transA(string_copy(aa,8,4)),o_ice)}
        if ai=31 {switch(string_copy(aa,12,1)) {
                case '0': ae=room_instance_add(argument0,transA(string_copy(aa,4,4)),transA(string_copy(aa,8,4)),obj_halfwall);break;
                case '1': ae=room_instance_add(argument0,transA(string_copy(aa,4,4)),transA(string_copy(aa,8,4)),obj_halfground);break;
                case '2': ae=room_instance_add(argument0,transA(string_copy(aa,4,4)),transA(string_copy(aa,8,4)),obj_static);break;
        }
        }
        if ai=32 {switch(string_copy(aa,17,1)) {
                case '0': ae=room_instance_add(argument0,transA(string_copy(aa,4,4)),transA(string_copy(aa,8,4)),o_waterchanger);skript=string_insert('('+string(ae)+').target='+string(transA(string_copy(aa,12,4)))+';('+string(ae)+').velocity='+string_copy(aa,16,1)+';('+string(ae)+').reuse=1;',skript,string_length(skript)+1)break;
                case '1': ae=room_instance_add(argument0,transA(string_copy(aa,4,4)),transA(string_copy(aa,8,4)),o_waterchanger);skript=string_insert('('+string(ae)+').target='+string(transA(string_copy(aa,12,4)))+';('+string(ae)+').velocity='+string_copy(aa,16,1)+';('+string(ae)+').reuse=0;',skript,string_length(skript)+1)break;
                case '2': ae=room_instance_add(argument0,transA(string_copy(aa,4,4)),transA(string_copy(aa,8,4)),o_waterregion);skript=string_insert('('+string(ae)+').target='+string(transA(string_copy(aa,12,4)))+';('+string(ae)+').velocity='+string_copy(aa,16,1)+';('+string(ae)+').endx='+string(transA(string_copy(aa,18,4)))+';('+string(ae)+').endy='+string(transA(string_copy(aa,22,4)))+';',skript,string_length(skript)+1)break;
        }
        }
        // ↓原来的
        ...
        // ↓现在的
        if ai=33 {
            ae=room_instance_add(argument3,transA(string_copy(aa,4,4)),transA(string_copy(aa,8,4)),o_bgmchange);
        ...
        if ai=34 {ae=room_instance_add(argument0,transA(string_copy(aa,4,4)),transA(string_copy(aa,8,4)),o_region);skript=string_insert('('+string(ae)+').endx='+string(transA(string_copy(aa,12,4)))+';'+'('+string(ae)+').endy='+string(transA(string_copy(aa,16,4)))+';',skript,string_length(skript)+1)}
    }
    if string_char_at(aa,1)='3' {
        ai=real(string_copy(aa,2,2))
        if ai>0 && ai<7 {ae=room_instance_add(argument0,transA(string_copy(aa,4,4)),transA(string_copy(aa,8,4)),o_pointblock);skript=string_insert('('+string(ae)+').bonus='+string(ai)+';',skript,string_length(skript)+1)}
        if ai>6 && ai<13 {ae=room_instance_add(argument0,transA(string_copy(aa,4,4)),transA(string_copy(aa,8,4)),o_pointblock2);skript=string_insert('('+string(ae)+').bonus='+string(ai-6)+';',skript,string_length(skript)+1)}
        if ai>18 && ai<25 {ae=room_instance_add(argument0,transA(string_copy(aa,4,4)),transA(string_copy(aa,8,4)),o_pointblock);skript=string_insert('('+string(ae)+').bonus='+string(ai+82)+';',skript,string_length(skript)+1)}
        switch(ai) {
        case(13):ae=room_instance_add(argument0,transA(string_copy(aa,4,4)),transA(string_copy(aa,8,4)),o_pointblock);skript=string_insert('('+string(ae)+').bonus=0;',skript,string_length(skript)+1);break;
        case(14):ae=room_instance_add(argument0,transA(string_copy(aa,4,4)),transA(string_copy(aa,8,4)),o_pointblock2);skript=string_insert('('+string(ae)+').bonus=0;',skript,string_length(skript)+1);break;
        case(15):ae=room_instance_add(argument0,transA(string_copy(aa,4,4)),transA(string_copy(aa,8,4)),o_blockbumper);skript=string_insert('('+string(ae)+').block=1;('+string(ae)+').bump_count=16;('+string(ae)+').bumped=1',skript,string_length(skript)+1);break;
        case(16):ae=room_instance_add(argument0,transA(string_copy(aa,4,4)),transA(string_copy(aa,8,4)),o_blockbumper);break;
        case(17):ae=room_instance_add(argument0,transA(string_copy(aa,4,4)),transA(string_copy(aa,8,4)),o_pointblock2);skript=string_insert('('+string(ae)+').bonus=7;',skript,string_length(skript)+1);break;
        case(18):ae=room_instance_add(argument0,transA(string_copy(aa,4,4)),transA(string_copy(aa,8,4)),o_point);break;
        case(25):ae=room_instance_add(argument0,transA(string_copy(aa,4,4)),transA(string_copy(aa,8,4)),o_pointblock);skript=string_insert('('+string(ae)+').bonus=7;',skript,string_length(skript)+1);break;
        case(26):ae=room_instance_add(argument0,transA(string_copy(aa,4,4)),transA(string_copy(aa,8,4)),o_pointblock2);skript=string_insert('('+string(ae)+').bonus=8;',skript,string_length(skript)+1);break;
        case(27):ae=room_instance_add(argument0,transA(string_copy(aa,4,4)),transA(string_copy(aa,8,4)),o_pointblock);skript=string_insert('('+string(ae)+').bonus=107;',skript,string_length(skript)+1);break;
        }
    ...
    if string_char_at(aa,1)='4' {
        if real(string_copy(aa,18,1))!=1 {
            ae=room_instance_add(argument0,transA(string_copy(aa,2,4)),transA(string_copy(aa,6,4)),o_exiter)} else
        {ae=room_instance_add(argument0,transA(string_copy(aa,2,4)),transA(string_copy(aa,6,4))-32,o_exiter)}
...
if ah=0 {show_message("Hey guy, please add a mario first :v");room_instance_add(argument3,real("0"),real("0"),o_marker)}

file_text_close(global.toload)
room_set_code(argument0,skript)
skript=''
