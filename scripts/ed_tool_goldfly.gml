...
if tool_mode=3 {draw_sprite(s_troopagoldfly,0,(floor(mouse_x/32))*32+16,(floor(mouse_y/32))*32+16);}//draw_text(fofo.x+4,fofo.y+4,global.agspeed)
if tool_mode=3 && mouse_check_button(mb_left) && clicked=0 && (global.fotel<>(floor(mouse_x/32))*32 || global.fotel2<>(floor(mouse_y/32))*32) {
    tool_mode=4;
...
if tool_mode=4 {
    ...
    if mouse_wheel_up() {global.goldcount+=1;
        ...
        tmp2.additional3=global.goldcount
    }
...
//左键确认落定
if tool_mode=4 && mouse_check_button(mb_left) && clicked=0 {
    tool_mode=0
    tmp2.test2=2
    clicked=1
    // NET-SYNC: 完成点——金飞龟落定（rotoord/rotor/rota/rotoc/rotod 全轨道）
    ed_net_ops_send_update(tmp2, 8)
}
