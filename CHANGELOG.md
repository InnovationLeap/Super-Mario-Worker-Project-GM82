...
## v1.8.0-beta.2 (2026-09-01)
### 关卡功能
- 游玩时内置游戏暂停功能。本版本起BGM将随游戏暂停而暂停。
- 在Play Level、Play Scenario暂停时，需长按1秒Yes才能退出关卡，防止误触。
### 编辑功能
- Edit部分界面进行重构。
### 问题修复
- 修复浣熊装水中不能播放行走动画的问题。
- 修复浣熊装飞行状态中即死后飞行音效滞留的问题。
- 修复Edit选区不能直接选中水管出口的问题。
- 修复文字绘制扩展在有很多文字的场景导致游戏拖慢的性能问题。
- 修复文字绘制扩展兼容模式下的字距问题。
### 其他
- 游戏生成的关卡版本号从1713提升至1714。

## v1.8.0-beta.1 (2026-08-16)
### 编辑功能
- 支持使用 `PgUp` / `PgDown` 键在起点、Check Point 的所在位置之间跳转。
- 支持在全局设置中修改编辑界面的快捷键。
- 支持自定义截图快捷键、截图保存路径（保存路径需要在 `GameSettings.ini` 中手动修改）。
- 全局设置新增“保存关卡时是否压缩”选项，默认开启。关闭该选项后，保存关卡时将跳过 GZip 压缩，直接保存文件。
- 新增选区功能，选择后可移动/复制选区内的物件。
- 新增多人联机功能，支持最多 8 名玩家同时编辑同一关卡。
- v1.7.12 及以下版本的关卡文件在使用此版本保存时，会自动生成一个备份副本。
### 关卡功能
- 新增半开放式强制滚屏，相对节点运动垂直部分的滚屏是自由的。
- 新增道具叶子（浣熊装）。
- 新增敌人静止刺壳、运动刺壳（龟壳类）。
### 问题修复
- 修复石盾不嘲讽的问题。
- 修复自定义音乐 ID 越界（超出音乐包列表、或音乐包缺失）时关卡报错崩溃的问题，现会静默跳过。
- 修复进入关卡或死亡重开时按住的方向、奔跑等按键状态丢失（开局按死不生效）的问题。
- 修复变身动画不平滑的问题。

## v1.7.13-beta1 (2026-07-25)
### 核心改进
- 将主程序引擎升级至 Game Maker 8.2。
- 使用全新的信息文本绘制插件，彻底修复部分新硬件设备上无法启动 SMWP 的问题。
- 使用全新的 SMWGZip DLL，程序不再依赖 .NET Framework 4 运行时。
- 新增文本扩展，支持将 GB2312 编码文本转换为 UTF-8。新版 SMWP 将默认使用 UTF-8 编码保存关卡。
### 编辑功能
- 新增编辑界面缩放功能，关闭侧边面板后，可使用 `Ctrl + 鼠标滚轮` 或 `+` / `-` 键对编辑界面进行缩放。
### 关卡功能
- 新增花瓣探照灯。
...
- 回退了 v1.7.12 将游玩操作代码中的 `keyboard_check` 替换为 `keyboard_check_direct` 的改动，该改动引入了部分体验问题。
- 修复了倒向和右向扎地食人花位置偏移 1 像素的问题。
### 已知问题
- 对于此前无法启动 SMWP v1.7、并在升级至 v1.7.12.1 后恢复正常运行的电脑，游戏内字体的锐度可能明显偏高，导致显示观感欠佳；此前本身能够正常运行 SMWP 的电脑不受此问题影响。
...
- 在运行关卡时，取消掉了原来的“按 F11 进入 Smooth 模式”。取而代之的是，按 F11 能够切换“是否启用敌人优化”。所谓“敌人优化”，是指 1.7.10 版针对敌人过多造成卡顿问题的相关优化，能有效改善大型关卡的卡顿问题。此项优化的副作用是，敌人与实心重叠时的一些行为会与旧版本不一样，若利用了此类特性制作关卡，游玩时需关闭敌人优化。1.7.10 及以上版本默认开启敌人优化，以下版本则默认关闭。按住 F1 可以查看是否开启敌人优化。
### 问题修复
...
### 关卡功能
- 开关砖和水砖现在可以顶死敌人了。
- 马里奥遇到虚变实的开关砖，现在会被卡在原处，不能操作。由实变虚后，恢复原有的水平速度和纵向速度。
### 问题修复
...
- 加强了火球龟。(2018-02-02)
### 问题修复
- 修复了无敌状态下踩不死乌龟等敌人的问题。(2018-02-02)
